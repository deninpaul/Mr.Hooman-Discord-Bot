...
const MISSING_QUERY_MESSAGE = new Discord.MessageEmbed()
  ...
  .setColor('#FAC11B')
  .setFooter(`Type \`${config.prefix + "help play"}\` to learn more`);
...
const ADDED_QUEUE_MESSAGE = function(song) {
  const newEmbed = new Discord.MessageEmbed()
    ...
    .setDescription(`Try \`${config.prefix}autoqueue\` to automatically fill up the queue with songs related to the current song!`)
...
const NOW_PLAYING_MESSAGE = function(guild, song) {
  ...
  const newEmbed = new Discord.MessageEmbed()
    ...
    .setAuthor('Now Playing', emotes.swag)
    .setThumbnail(song.thumbnail)
    .setColor('#FAC11B')
    .addField(`\`${progressBarView}\``, `\`${dispTime(duration)} - ${dispTime(song.duration)}\``)
    .setFooter(`Requested By: ${song.requested}. Try ${config.prefix}autoqueue`)
